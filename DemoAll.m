clc;
clear;
path = './'; addpath(genpath(path));
warning off;

datasetname = cell(10 , 1);
datasetname{1}='yaleA_3view';
datasetname{2}='MSRCV1';
datasetname{3}='3sources';
datasetname{4}='ORL_mtv';
datasetname{5}='proteinFold';
datasetname{6}='WebKB_cor2views';
datasetname{7}='WebKB_Wisconsin2views';
datasetname{8}='bbcsport_seg14of4';
datasetname{9}='WebKB';
datasetname{10}='WebKB_2views';
datasetname{11}='WikipediaArticles';
datasetname{12}='Caltech101-7';
datasetname{13}='Caltech101-20';
datasetname{14}='Handwritten_numerals';
...
for di = 14:14  % Datasets
    datapath = './0datasets/';
    dataName = datasetname{di};
    load([datapath,'/',dataName,'.mat'],'X','Y');
    ...
    numiter = 50;
    Result_1Co_pw = zeros(length(lambda),8);
    for li =1:length(lambda)
        % fprintf('1Co-regularized-Pairwise: Multiview spectral with 2 views \n ');
        tic;
        [Result_1Co_pw(li,:)] = spectral_pairwise_multview(data,viewN,k,sigma,lambda(li),Y,numiter);
        t3(li) = toc;
    end
    Allresult(3,:) = max(Result_1Co_pw,[],1);
