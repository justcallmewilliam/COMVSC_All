function [ress, H] = HSIC(X,gt,cls_num,lambda1,lambda2,K,epsilon,alpha)
%% Initialize P,H,Z,E_h,E_r,J,F and Y_1,Y_2,Y_3,Y_4
V = length(X); %nunber of views
...
while (IsConverge == 0&&iter<=max_iter)
    ...
    % update J
    J = softth((Z+Y2/mu)+eye(N)*1e-8,lambda1/mu);    %lambda1û�ж���
