...
datasetname{2}='3sources';
datasetname{3}='ORL_mtv';
datasetname{4}='proteinFold';
datasetname{5}='WebKB';
datasetname{6}='WebKB_2views';
datasetname{7}='WebKB_cor2views';

datasetname{8}='WebKB_Wisconsin2views'; %

datasetname{9}='MSRCV1'; % 
datasetname{10}='WikipediaArticles'; % jiyuan
datasetname{11}='Caltech101-7'; % tsl
datasetname{12}='Caltech101-20'; % hzc

datasetname{13}='Handwritten_numerals'; %
datasetname{14}='bbcsport_seg14of4'; %
...
%             B{vnum} = (1/lmd(i))*(I_n{vnum}-(1/lmd(i))*X{vnum}'*inv(I_d{vnum}+(1/lmd(i))*X{vnum}*X{vnum}')*X{vnum});
        ...
        for j=1:length(gma)
            disp(['Para1=',num2str(lmd(i)),'  Para2=',num2str(gma(j))]);
    ...
    save(['./Result1012_Our_' , dataName , '.mat'], 'Allresult', 'Result_11COMVSC',...
        'ourFscore', 'ourPrecision', 'ourRecall' ,'ourNmi', 'ourAR', 'ourEntropy', 'ourACC', 'ourPurity');
    dlmwrite(['TxtResultOfOur_', dataName, '.txt'], Result_11COMVSC,'-append','delimiter','\t','newline','pc');
