function [ressmean] = spectral_centroid_multiview(X,num_views,numClust,sigma,lambda,truth,numiter)
% INPUT:
% OUTPUT:
    ...
    for j=1:kmeans_avg_iter
        ...
        [nmi_j(j) avgent_j(j)] = compute_nmi(truth,C);
    ...
    while(i<=numiter+1)
        ...
        if (1)  %use view 1 in actual clustering
            ...
            for j=1:kmeans_avg_iter
                ...
                [nmi_j(j) avgent_j(j)] = compute_nmi(truth,C);
                [ARj(j),RIj(j),MIj(j),HIj(j)]=RandIndex(truth,C);
                ress(j,:) = Clustering8Measure(truth,C);
            end
